...
On invocation, the skill fetches the PR diff via `gh pr diff` and `gh pr view`, snapshots it into a session directory, and starts a headless session the IntelliJ plugin discovers by cwd. A persistent background watcher monitors for events. When the user submits a question on a line (from the IDE), the watcher wakes Claude with the anchor (file, side, line number) and the question text. Claude reads the diff context, composes a short answer, and appends it to the line's thread file. The IDE streams new thread entries over SSE and refreshes the thread in place.
