...
def test_the_busy_banner_has_no_empty_sub_label():
    ...
    progress line. Nothing could write it: the banner's labels came from the
    PostToolUse hook `skills/annotate/hooks/progress_publish.py`, which
    mapped tool names onto a fixed allowlist and knew nothing about mark
    counts. There is no `.bb-*` CSS either, so the node shipped as an empty
    flex child eating a `gap: 9px` slot.

    That hook and its registration are now deleted, and narration is written
    by Claude through `skills/annotate/progress.py` and painted by the
    panel — which carries the step count the sub-label was meant to. The
    ribbon itself keeps no sub-label, and this test keeps it that way.
    """
