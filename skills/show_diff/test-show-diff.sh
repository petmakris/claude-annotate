...
#
# The script's whole job is to reach VS Code, so every case here runs it against stubs
# for `code`, `open` and `osascript` placed ahead of the real ones on PATH. The stub
# `open` records the URI it was handed, which is the script's actual output — everything
# printed to the terminal is commentary on it.
set -u
...
STUB
chmod +x "$BIN/code" "$BIN/open" "$BIN/osascript"
