...
2. `skills/ask_diff/server.py` and `skills/walkthrough/server.py` — the two remaining per-skill `Handlers` classes, in particular each `serve_data()`, where their routes actually live (see Step 2's third form).
