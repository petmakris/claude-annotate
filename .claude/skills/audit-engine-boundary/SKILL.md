...
2. `skills/ask_diff/server.py` — the one remaining consumer.
...
- **Rule 4 — port ranges stay disjoint.** `PORT_RANGE` across skill servers must not overlap. Today: ask-diff 54620–54640 is the only one left running its own server. An overlap is **Critical** — two skills race for a port and the loser reports a stale one.
...
6. `skills/ask_diff/server.py`'s `_serve_stream` HTTP transport framing — the browser-facing SSE loop. The engine has no SSE-serving module for it to reimplement, so this is skill-owned, not a Rule 1 Violation.
