...
description: Audit the shared server engine boundary — that `skills/_shared/web_companion/` is the one copy, reached by import rather than reimplemented, and that skill servers do not collide on ports or state directories. Finds hand-rolled reimplementations of engine behaviour, resurrected vendoring artifacts, overlapping PORT_RANGEs and duplicate skill_names. Reports in plain English. Use when the user says "/audit-engine-boundary", "check the engine boundary", or asks whether the shared server is still shared.
user-invocable: true
---
...
2. `skills/interactive_review/server.py` — the one remaining consumer.
...
- **Rule 4 — port ranges stay disjoint.** `PORT_RANGE` across skill servers must not overlap. Today: interactive-review 54620–54640 is the only one left running its own server. An overlap is **Critical** — two skills race for a port and the loser reports a stale one.
...
1. The one remaining per-skill `server.py` **handler** module. It is meant to be distinct — the engine owns transport, the skill owns its routes' meaning.
2. The 8-line per-skill `ensure_server.sh` shim. It deliberately delegates to the 153-line one in `_shared`.
...
6. `skills/interactive_review/server.py`'s `_serve_stream` HTTP transport framing — the browser-facing SSE loop. The engine has no SSE-serving module for it to reimplement, so this is skill-owned, not a Rule 1 Violation.
...
Enumerate engine modules and the public names each exports. Grep the skill server(s) for those names to find who imports what. Grep the same files for the behaviours in Rule 1 implemented locally. Extract each `PORT_RANGE` and each `run(skill_name=...)` argument and diff them. Build the file universe from `git ls-files`.
